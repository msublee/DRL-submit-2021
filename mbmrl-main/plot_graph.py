...
def save_graph():
    ...
    if plot_avg:
        ...
        data_avg['reward'] = data_avg['reward'].rolling(window=1, win_type='triang',
                                                        min_periods=1).mean()

        data_avg.plot(kind='line', x='timestep', y='reward_smooth', ax=ax, color=colors[int(env_name[-1])],
                      linewidth=linewidth_smooth,
                      alpha=alpha_smooth)
        # plt.fill_between(data_avg['timestep'], data_avg['reward_smooth'] - data_avg['reward_var'],
        #                  data_avg['reward_smooth'] + data_avg['reward_var'], color=colors[0], alpha=alpha_var)
        data_avg.plot(kind='line', x='timestep', y='reward_var', ax=ax, color=colors[int(env_name[-1])],
                      linewidth=linewidth_var,
                      alpha=alpha_var)
        data_avg.plot(kind='line', x='timestep', y='reward', ax=ax, color=colors[int(env_name[-1])],
                      linewidth=2.5,
                      alpha=0.2)
