...
def train():
    ...
    warmup = 10
    if env_name == "CartPoleSwingUp-v2":
        max_training_timesteps = int(1e5) + warmup  # break training loop if timeteps > max_training_timesteps
    else:
    ...
    action_std_decay_freq = int(8000)  # action_std decay frequency (in num timesteps)
    ...
    batch = AttrDict()
    batch.xc = torch.tensor([], device=device)
    batch.yc = torch.tensor([], device=device)
